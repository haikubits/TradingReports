...
def fetch_yahoo_chart_history(ticker_str: str, data_range: str, interval: str) -> pd.DataFrame:
    from urllib.parse import quote
    from urllib.request import Request, urlopen

    url = f"https://query1.finance.yahoo.com/v8/finance/chart/{quote(ticker_str, safe='')}?range={data_range}&interval={interval}"
    ...
    history = pd.DataFrame(quote_data)
    history.index = pd.to_datetime(timestamps, unit="s", utc=True).tz_convert(ET)
...
def fetch_raw_data(ticker_str: str) -> dict[str, Any]:
    ...
    if daily.empty:
        daily = fetch_yahoo_chart_history(ticker_str, "1y", "1d")
    if daily.empty:
...
def fetch_vix() -> float:
    ...
    if not history.empty:
        return safe_number(history["Close"].iloc[-1])

    # Yahoo's chart endpoint is a narrow fallback when yfinance rejects the index symbol.
    from urllib.request import Request, urlopen
    ...
    if latest_close is None:
        raise ValueError("No VIX close returned from Yahoo Finance")
    return safe_number(latest_close)
...
<div class="page">
  ...
  {failures_html}<div class="footer-note">This report is for educational/research purposes only. Not financial advice. All levels are generated from technical analysis. Past performance does not guarantee future results.</div>
</div>
<script>
