...
def build_report_data(universe: list[str] | None = None, top_n: int = 5) -> dict[str, Any]:
    ...
    vix = fetch_vix()
    vix_regime = "unavailable" if vix is None else "low" if vix < 15 else "moderate" if vix < 20 else "elevated" if vix < 30 else "high"
...
def generate_html(report_data: dict[str, Any]) -> str:
    ...
    vix_tone = "success" if report_data["vix_regime"] == "low" else "warning" if report_data["vix_regime"] in {"moderate", "unavailable"} else "danger"
    ...
    context_chips = "".join([
        ...
        render_context_chip("VIX", f"{report_data['vix']:.1f}" if report_data["vix"] is not None else "Unavailable", vix_tone),
