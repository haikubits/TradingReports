...
def fetch_raw_data(ticker_str: str) -> dict[str, Any]:
    ...
    if not math.isfinite(last_price) or last_price <= 0:
        raise ValueError(f"Invalid close data returned for {ticker_str}")
...
def compute_levels(hourly: pd.DataFrame, daily: pd.DataFrame, current_price: float, ntz: dict[str, float]) -> dict[str, Any]:
    ...
    valid_levels = [level for level in level_map.values() if math.isfinite(level["price"])]
    ...
    above = sorted([v for v in valid_levels if v["price"] > current_price], key=lambda item: item["price"])[:3]
    below = sorted([v for v in valid_levels if v["price"] < current_price], key=lambda item: item["price"], reverse=True)[:3]
