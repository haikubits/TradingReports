...
YAHOO_HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; TradingReportsScanner/1.0)"}


def fetch_chart_history(ticker_str: str, period: str, interval: str, prepost: bool = False) -> pd.DataFrame:
    response = requests.get(
        YAHOO_CHART_URL.format(ticker=ticker_str),
        params={"range": period, "interval": interval, "includePrePost": str(prepost).lower(), "events": "history"},
    ...
    if payload.get("error"):
        raise ValueError(f"Yahoo chart error for {ticker_str}: {payload['error']}")
    results = payload.get("result") or []
    if not results:
        raise ValueError(f"No chart data returned for {ticker_str}")
    result = results[0]
...
def fetch_raw_data(ticker_str: str) -> dict[str, Any]:
    daily = fetch_chart_history(ticker_str, "1y", "1d")
    hourly = fetch_chart_history(ticker_str, "1mo", "60m", prepost=True)
    intraday = fetch_chart_history(ticker_str, "1d", "5m")

    if daily.empty:
        raise ValueError(f"No daily data returned for {ticker_str}")

    # The options endpoint requires a crumb token in this network, so preserve the
    # existing non-fatal behavior and score options volume as unavailable.
    options = None
...
def fetch_vix() -> float:
    history = fetch_chart_history("^VIX", "5d", "1d")
