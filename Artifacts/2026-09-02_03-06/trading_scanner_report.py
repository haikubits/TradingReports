...
UTC = ZoneInfo("UTC")
# The sandbox's outbound proxy certificate is not accepted by curl-cffi's default verifier.
MARKET_DATA_SESSION = curl_requests.Session(impersonate="chrome", verify=False)
...
def fetch_raw_data(ticker_str: str) -> dict[str, Any]:
    tk = yf.Ticker(ticker_str, session=MARKET_DATA_SESSION)
...
def fetch_vix() -> float:
    vix = yf.Ticker("^VIX", session=MARKET_DATA_SESSION)
