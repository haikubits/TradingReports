...
def build_report_data(universe: list[str] | None = None, top_n: int = 5) -> dict[str, Any]:
    ...
    vix_regime = (
        "unavailable"
        if vix is None
        else "low" if vix < 15 else "moderate" if vix < 20 else "elevated" if vix < 30 else "high"
    )
...
def generate_html(report_data: dict[str, Any]) -> str:
    ...
    vix_text = "Unavailable" if report_data["vix"] is None else f"{report_data['vix']:.1f}"
    ...
    context_chips = "".join([
        ...
        render_context_chip("VIX", vix_text, vix_tone),
